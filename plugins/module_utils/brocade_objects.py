...
from ansible_collections.brocade.fos.plugins.module_utils.brocade_access_gateway import (
    to_fos_access_gateway_policy,
    to_human_access_gateway_policy,
)
from ansible_collections.brocade.fos.plugins.module_utils.brocade_chassis import chassis_get, chassis_patch
from ansible_collections.brocade.fos.plugins.module_utils.brocade_connection import exit_after_login, login, logout
from ansible_collections.brocade.fos.plugins.module_utils.brocade_fibrechannel_configuration import (
    fabric_get,
    fabric_patch,
    port_configuration_get,
    port_configuration_patch,
)
from ansible_collections.brocade.fos.plugins.module_utils.brocade_fibrechannel_switch import (
    fc_switch_get,
    fc_switch_patch,
    to_fos_switch,
    to_human_switch,
)
from ansible_collections.brocade.fos.plugins.module_utils.brocade_interface import (
    fc_port_get,
    fc_port_patch,
    to_fos_fc,
    to_human_fc,
...
from ansible_collections.brocade.fos.plugins.module_utils.brocade_yang import (
    generate_diff,
    human_to_yang,
    is_full_human,
    str_to_human,
...
def sanitize(data, sensitive_keys=None):
    """
    Sanitize sensitive data in dictionaries, lists, and XML strings.

    Replaces sensitive key values in dicts/lists and password content in XML
    with VALUE_SPECIFIED_IN_NO_LOG_PARAMETER placeholder.

    Args:
        data: Dictionary, list, or XML string to sanitize
        sensitive_keys: List of sensitive key names to mask (default: ['password'])

    Returns:
        Sanitized data in the same format as input
    """
    return data
    ...
    #     return re.sub(
    #         r"(?P<open_tag><password>)(?P<content>.*?)(?P<close_tag></password>)",
    #         r"\g<open_tag>VALUE_SPECIFIED_IN_NO_LOG_PARAMETER\g<close_tag>",
    #         data,
    #         flags=re.DOTALL
    #     )
    ...
    #         if key in sensitive_keys or key_normalized in sensitive_keys:
    #             sanitized[key] = "VALUE_SPECIFIED_IN_NO_LOG_PARAMETER"
    #         elif isinstance(value, (dict, list)):
    #             sanitized[key] = sanitize(value, sensitive_keys)
    #         else:
    #             sanitized[key] = value
    #     return sanitized
    # elif isinstance(data, list):
    #     return [sanitize(item, sensitive_keys) for item in data]
    # else:
    #     return data
...
def get_moduleName(fos_version, module_name):
    ...
    if module_name == "brocade_fibrechannel_switch" or module_name == "switch":
        result = "switch" if ifos_version < 9 else "brocade_fibrechannel_switch"
    elif module_name == "brocade_fibrechannel_logical_switch" or module_name == "logical_switch":
        result = "logical_switch" if ifos_version < 9 else "brocade_fibrechannel_logical_switch"
    elif module_name == "brocade_fibrechannel_diagnostics" or module_name == "diagnostics":
        result = "diagnostics" if ifos_version < 9 else "brocade_fibrechannel_diagnostics"
    elif module_name == "brocade_fabric" or module_name == "fabric":
        result = "fabric" if ifos_version < 9 else "brocade_fabric"
    else:
        result = module_name
...
def to_human_singleton(module_name, obj_name, attributes):
    ...
    if (
        (module_name == "brocade_time" and obj_name == "clock_server")
        and ("ntp_server_address" in attributes and "server_address" in attributes["ntp_server_address"])
        and not isinstance(attributes["ntp_server_address"]["server_address"], list)
    ):
        new_list = []
...
def to_fos_singleton(module_name, obj_name, attributes, result):
    ...
    for k, v in attributes.items():
        ...
        if (
            ...
            and (
                obj_name == "security_certificate_action"
                or obj_name == "sshutil_public_key_action"
                or obj_name == "sec_crypto_cfg_template_action"
            )
            and k == "remote-user-password"
        ):
            attributes[k] = to_base64(attributes[k])
...
def singleton_get(
    ...
    full_url, validate_certs = full_url_get(is_https, fos_ip_addr, REST_PREFIX + module_name + "/" + obj_name)

    ret, resp = url_get_to_dict(fos_ip_addr, is_https, auth, vfid, result, full_url, timeout)
...
def to_human_list(module_name, list_name, attributes_list, result):
    for attributes in attributes_list:
        ...
        if module_name == "brocade_security" and list_name == "user_config":
            vfr = attributes.get("virtual_fabric_role_id_list")
            if vfr is not None and "role_id" in vfr:
        ...
        if (
            module_name == "brocade_fibrechannel_switch" or module_name == "switch"
        ) and list_name == "fibrechannel_switch":
            ...
            if "ip_static_gateway_list" in attributes:
                ipgw = attributes["ip_static_gateway_list"]
                if ipgw is not None and "ip_static_gateway" in ipgw:
        ...
        if module_name == "brocade_access_gateway" and list_name == "port_group":
            if "port_group_n_ports" in attributes:
                pgn = attributes["port_group_n_ports"]
                if pgn is not None and "n_port" in pgn:
            ...
            if "port_group_f_ports" in attributes:
                pgf = attributes["port_group_f_ports"]
                if pgf is not None and "f_port" in pgf:
        ...
        if module_name == "brocade_access_gateway" and list_name == "n_port_map":
            if "configured_f_port_list" in attributes:
                cfl = attributes["configured_f_port_list"]
                if cfl is not None and "f_port" in cfl:
        ...
        if module_name == "brocade_maps" and list_name == "rule":
            ...
            if "actions" in attributes:
                if (attributes["actions"] is not None and "action" in attributes["actions"]) and not isinstance(
                    attributes["actions"]["action"], list
                ):
                    new_list = []
...
def to_fos_list(module_name, list_name, attributes_list, result):
    for attributes in attributes_list:
        ...
        if module_name == "brocade_security" and list_name == "user_config" and "password" in attributes:
...
def list_entry_keys(module_name, list_name):
    if module_name in list_keys and list_name in list_keys[module_name]:
...
def singleton_patch(
    ...
    if module_name == "brocade_fibrechannel_configuration" and obj_name == "port_configuration":
        return port_configuration_patch(
            login,
            password,
            fos_ip_addr,
            fos_version,
    ...
    result["patch_obj_str"] = sanitize(xml_str)
        ...
        for k, v in entry.items():
            if str_to_human(k) not in list_entry_keys(module_name, list_name):
                ...
                if isinstance(v, dict):
                    ...
                    xml_str = xml_str if v is None else xml_str + str(v)
...
def list_patch(
    ...
    if module_name == "brocade_security" and list_name == "user_config":
        return user_config_patch(
            login, password, fos_ip_addr, fos_version, is_https, auth, vfid, result, entries, ssh_hostkeymust, timeout
        )

    if module_name == "brocade_snmp" and list_name == "v1_trap":
        new_entries = v1_trap_patch(
            login, password, fos_ip_addr, fos_version, is_https, auth, vfid, result, entries, ssh_hostkeymust, timeout
        )
    ...
    result["patch_str"] = sanitize(xml_str)
    ...
    if module_name == "brocade_access_gateway" and list_name == "port_group":
        ...
        for port_group in entries:
            pgn = port_group.get("port-group-n-ports")
            if pgn is not None and "n-port" in pgn and pgn.get("n-port") is not None:
                empty_port_groups.append(
                    {"port-group-id": port_group["port-group-id"], "port-group-n-ports": {"n-port": None}}
                )
            pgf = port_group.get("port-group-f-ports")
            if pgf is not None and "f-port" in pgf and pgf.get("f-port") is not None:
                empty_port_groups.append(
                    {"port-group-id": port_group["port-group-id"], "port-group-f-ports": {"f-port": None}}
                )
    ...
    if module_name == "brocade_access_gateway" and list_name == "n_port_map":
        ...
        for n_port_map in entries:
            if (
                "configured-f-port-list" in n_port_map
                and "f-port" in n_port_map["configured-f-port-list"]
                and n_port_map["configured-f-port-list"]["f-port"] is not None
            ):
...
def list_post(
    ...
    result["post_str"] = sanitize(xml_str)
...
def list_delete(
    ...
    result["delete_str"] = sanitize(xml_str)
...
def singleton_helper(
            ...
            if not module.check_mode:
                if module_name == "brocade_access_gateway" and obj_name == "policy" and force is True:
                    if "auto-policy-enabled" in diff_attributes and diff_attributes["auto-policy-enabled"] == "1":
                        ...
                        "port-group-policy-enabled" in diff_attributes
                        and diff_attributes["port-group-policy-enabled"] == "1"
                    ):
                        switch_module = get_moduleName(fos_version, "brocade_fibrechannel_switch")
                        switch_obj = "fibrechannel_switch"
                        ret_code, response = singleton_get(
    ...
    if (
        module_name == "brocade_security"
        and obj_name == "security_certificate_action"
        and len(diff_attributes) > 0
        and diff_attributes["certificate-type"] == "https"
        and diff_attributes["operation"] == "import"
    ):
...
def list_helper(
    ...
    try:
        ...
        if not isinstance(current_entries, list):
            current_entries = [] if current_entries is None else [current_entries]
        ...
        if module_name == "brocade_fibrechannel_logical_switch" and list_name == "fibrechannel_logical_switch":
            new_current_entries = []
            for current_entry in current_entries:
                ...
                if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                    ...
                    if len(diff_attributes) > 0:
                        ...
                        if (
                            ...
                            and "port_member_list" in entry
                            and "port_member_list" in diff_attributes
                        ):
                            continue
        ...
        for entry in entries:
            ...
            for current_entry in current_entries:
                if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                    ...
                    if list_name == "fibrechannel_logical_switch" and "port_member_list" in entry:
                        ...
                        if (
                            len(add_diff_attributes) > 0
                            and "port_member_list" in add_diff_attributes
                            and len(entry) == 2
                        ):
                            found_diff = True
                    break
            ...
            if not found or found_diff:
                new_entry = {}
                if found_diff:
                    new_entry["fabric_id"] = entry["fabric_id"]
                    new_entry["port_member_list"] = entry["port_member_list"]
                else:
                    for k, v in entry.items():
                        new_entry[k] = v
            ...
            for entry in entries:
                if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                    ...
                    if list_name == "fibrechannel_logical_switch" and "port_member_list" in entry:
                        ...
                        if (
                            len(delete_diff_attributes) > 0
                            and "port_member_list" in delete_diff_attributes
                            and len(entry) == 2
                        ):
                            found_diff = True
                    break
...
def _read_file(path, result, module):
    ...
    for line in lines:
        ...
        if line.startswith("|"):
                ...
                if current_fos_ansible:
                    if current_fos_version:
                        current_fos_versions.append(
                            {"version": current_fos_version, "tested_with": current_tested_with}
                        )
            ...
            if fos and current_fos_ansible:
                ...
                if current_fos_version:
                    current_fos_versions.append({"version": current_fos_version, "tested_with": current_tested_with})
...
def moduleCompatibility_helper(module, fos_ip_addr, fos_user_name, fos_password, https, throttle, result, timeout):
    ret_code, auth, fos_version = login(fos_ip_addr, fos_user_name, fos_password, https, throttle, result, timeout)
    if ret_code != 0:
        module.exit_json(**result)
    ...
    if Path(filename).exists():
        fcontent = _read_file(filename, result, module)
        ...
        if not Path(os.getenv("BROCADE_VERSION_PATH")).exists():
            ...
            fcontent = _read_file(os.getenv("BROCADE_VERSION_PATH"), result, module)
        elif (Path(os.getenv("BROCADE_VERSION_PATH")).parent / filename).exists():
            fcontent = _read_file(Path(os.getenv("BROCADE_VERSION_PATH")).parent / filename, result, module)
        else:
            result["failed"] = True
            result["response"] = f"{os.getenv('BROCADE_VERSION_PATH')} must point to {filename}"
            exit_after_login(fos_ip_addr, https, auth, result, module, timeout)
    elif os.getenv("ANSIBLE_LIBRARY"):  # colection installed using ANSIBLE_LIBRARY
        fcontent = _read_file(Path(os.getenv("ANSIBLE_LIBRARY")) / filename, result, module)
    elif (Path.home() / ".ansible.cfg").exists():  # ~/.ansible.cfg present
        fcontent = _read_file(Path.home() / ".ansible.cfg", result, module)
        if not _fos_module_defined(fcontent, result):
            if not Path("/etc/ansible/ansible.cfg").exists():  # /etc/ansible/ansible.cfg not present
                ...
                result["failed"] = True
                result["response"] = "Define 'BROCADE_VERSION_PATH' environment variable"
                exit_after_login(fos_ip_addr, https, auth, result, module, timeout)
            else:
                fcontent = _read_file("/etc/ansible/ansible.cfg", result, module)
                if not _fos_module_defined(fcontent, result):
                    result["failed"] = True
                    result["response"] = "Define 'BROCADE_VERSION_PATH' environment variable"
                    exit_after_login(fos_ip_addr, https, auth, result, module, timeout)
        fcontent = _read_file(result["versionFilePath"], result, module)
    ...
    try:
        ...
        for _collection_ver, collection_info in fcontent.items():
...
def list_operation(
    ...
    if len(op_entries) > 0:
        if not module.check_mode:
            result["input"] = sanitize(op_entries)
            if list_name == "vrf":
            ...
            while tcount < 1000:
                tcount += 10
                time.sleep(10)
                result["input"] = sanitize(attributes)
...
def list_operation_helper(
    ...
    if list_name == "staticArp" or list_name == "staticRoute" or list_name == "lag":
        ...
        if all_entries:
            for current_entry in current_entries:
                ...
                if list_name == "staticRoute" and "vrfID" not in deletefirst_attributes and "vrfID" in current_entry:
                ...
                for current_entry in current_entries:
                    if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                        ...
                        if (
                            ...
                            and "vrfID" not in deletefirst_attributes
                            and "vrfID" in current_entry
                        ):
                            deletefirst_attributes["vrfID"] = current_entry["vrfID"]
                        deletefirst_entries.append(deletefirst_attributes)
    ...
    diff_entries = []
    for entry in entries:
        if list_name == "configuration" and entry["trafficClassName"] in ["sysTcDefault", "sysTcAclDeny"]:
        ...
        for current_entry in current_entries:
            if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                ...
                if list_name == "configuration" and current_entry["trafficClassName"] == "sysTcDefault":
                    continue
                if list_name == "configuration" and current_entry["trafficClassName"] == "sysTcAclDeny":
                    continue
                diff_attributes = generate_diff(result, current_entry, entry)
                if len(diff_attributes) > 0:
                    if list_name == "vrf":
                        ...
                        if "dhcpEnabled" not in diff_attributes and "dhcpEnabled" in entry:
        ...
        for current_entry in current_entries:
            if list_entry_keys_matched(entry, current_entry, module_name, list_name):
                ...
                if list_name == "interface":
                    if (
                        "nativeVlanID" in entry
                        and "nativeVlanID" in current_entry
                        and entry["nativeVlanID"] == current_entry["nativeVlanID"]
                    ):
                        found = True
                    else:
                        found = False
...
def operation_helper(
    ...
    try:
        result["input"] = sanitize(attributes)
        # Create a copy to avoid mutating the original (which Ansible logs uses)
        import copy

        attributes_for_api = copy.deepcopy(attributes)

        ret_code = to_fos_operation(op_name, in_name, attributes_for_api, result)
        if ret_code != 0:
            exit_after_login(fos_ip_addr, https, auth, result, module, timeout)
...
def operation_xml_str(result, op_name, obj_name, attributes_list):
    ...
    if (
        op_name == "supportsave"
        or op_name == "firmwaredownload"
        or op_name == "show_status"
        or op_name == "configupload"
        or op_name == "configdownload"
        or op_name == "license"
    ):
        xml_str = xml_str + "<" + obj_name_yang + ">\n"
...
def operation_post(
    ...
    :rtype: list
    """
    full_url, validate_certs = full_url_get(is_https, fos_ip_addr, OP_PREFIX + op_name)
    ...
    return url_post_resp(fos_ip_addr, is_https, auth, vfid, result, full_url, xml_str, timeout)
...
def to_fos_operation(op_name, in_name, attributes_list, result):
    if (
        ...
        for k, v in attributes_list.items():
            # if going to fos, we need to encode password
            if op_name == "supportsave" and in_name == "connection" and k == "password" and v is not None:
                attributes_list[k] = to_base64(attributes_list[k])
            if op_name == "firmwaredownload" and in_name == "firmwaredownload_parameters":
                if k == "password" and v is not None:
                    attributes_list[k] = to_base64(attributes_list[k])
            if op_name == "license" and in_name == "license_parameters":
                if k == "password" and v is not None:
                    attributes_list[k] = to_base64(attributes_list[k])
