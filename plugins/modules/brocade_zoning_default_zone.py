...
from ansible_collections.brocade.fos.plugins.module_utils.brocade_zoning import (
    cfg_abort,
    cfg_save,
    effective_get,
    effective_patch,
...
def main():
    ...
    diff_attributes = {}
    if default_zone_access is not None and default_zone_access != resp_effective["default_zone_access"]:
    ...
    if len(diff_attributes) > 0:
        ...
        if not module.check_mode:
            ret_code = effective_patch(fos_ip_addr, https, fos_version, auth, vfid, result, diff_attributes, timeout)
            if ret_code != 0:
                exit_after_login(fos_ip_addr, https, auth, result, module, timeout)
