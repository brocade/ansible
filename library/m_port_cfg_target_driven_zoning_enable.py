...
#        })
    ...
    if pyfos_auth.is_failed_login(session):
        print (json.dumps({"changed": False,
            "login failed reason": session.get(pyfos_auth.CREDENTIAL_KEY)[pyfos_auth.LOGIN_ERROR_KEY]}))
        sys.exit()
    ...
    if result.peek_target_driven_zoning_enable() != int(mode):
        changed = True
        if checkmode is False:
            port = pyfos_switchfcport.fibrechannel()
            port.set_name(name)
            port.set_target_driven_zoning_enable(int(mode))
