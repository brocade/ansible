...
from ansible.module_utils.brocade_access_gateway import to_fos_access_gateway_policy, to_human_access_gateway_policy
...
from ansible.module_utils.brocade_fibrechannel_configuration import (
    fabric_get,
    fabric_patch,
    port_configuration_get,
    port_configuration_patch,
)
from ansible.module_utils.brocade_fibrechannel_switch import (
    fc_switch_get,
    fc_switch_patch,
    to_fos_switch,
    to_human_switch,
)
from ansible.module_utils.brocade_interface import fc_port_get, fc_port_patch, to_fos_fc, to_human_fc
...
from ansible.module_utils.brocade_url import (
    ERROR_LIST_EMPTY,
    full_url_get,
    url_delete,
    url_get_to_dict,
...
from ansible.module_utils.brocade_yang import (
    generate_diff,
    human_to_yang,
    is_full_human,
    str_to_human,
...
def to_human_singleton(module_name, obj_name, attributes):
    ...
    if (module_name == "brocade_time" and obj_name == "clock_server") and \
       ("ntp_server_address" in attributes and "server_address" in attributes["ntp_server_address"]) and \
       not isinstance(attributes["ntp_server_address"]["server_address"], list):
        new_list = []
...
def to_fos_singleton(module_name, obj_name, attributes, result):
    ...
    for k, v in attributes.items():
        ...
        if module_name == "brocade_security" and (
    ...
    for attributes in attributes_list:
        ...
        if module_name == "brocade_maps" and list_name == "rule":
            ...
            if "actions" in attributes:
                if (attributes["actions"] is not None and "action" in attributes["actions"]) and \
                        not isinstance(attributes["actions"]["action"], list):
                    new_list = []
