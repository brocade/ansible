...
def login(fos_ip_addr, fos_user_name, fos_password, is_https, throttle, result, timeout):
    ...
    Login to the fos switch at ip address specified
        ...
        :rtype: dict
    """
    full_login_url, validate_certs = full_url_get(is_https, fos_ip_addr, REST_LOGIN)
    ...
    retval, eret, edict, login_resp = url_helper(
        full_login_url, None, "POST", None, result, validate_certs, timeout, credential=credential
    )
    ...
    full_switch_url, validate_certs = full_url_get(is_https, fos_ip_addr, REST_SWITCH_NEW_URI)
    ...
    rtype, rdict = url_get_to_dict(fos_ip_addr, is_https, auth, -1, result, full_switch_url, timeout)
    if rtype != 0:
        # Try with old switch URI
        full_switch_url, validate_certs = full_url_get(is_https, fos_ip_addr, REST_SWITCH_URI)
        # get fos version from the default switch
        rtype, rdict = url_get_to_dict(fos_ip_addr, is_https, auth, -1, result, full_switch_url, timeout)
        if rtype != 0:
            result["failed"] = True
...
def logout(fos_ip_addr, is_https, auth, result, timeout):
    ...
    logout from the fos switch at ip address specified
        ...
        :rtype: int
    """
    full_logout_url, validate_certs = full_url_get(is_https, fos_ip_addr, REST_LOGOUT)

    return url_post(fos_ip_addr, is_https, auth, None, result, full_logout_url, None, timeout)
